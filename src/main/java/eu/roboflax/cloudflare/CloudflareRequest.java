...
public class CloudflareRequest {
	...
	public static final String ERROR_RESULT_IS_JSON_OBJECT = "Property 'result' is not a json array, because it is a json object"
			+ " use asObject() instead of asObjectList().";
	public static final String ERROR_RESULT_IS_JSON_ARRAY = "Property 'result' is not a json object, because it is a json array "
			+ "use asObjectList() instead of asObject().";
	...
	public static final String ERROR_CLOUDFLARE_FAILURE =
			"Cloudflare was unable to perform your request and couldn't determine the result of the requested/passed "
					+ "information. "
					+
	 ...
	 */
	private <T> void runCallback(CloudflareCallback<CloudflareResponse<T>> callback,
								 Callable<CloudflareResponse<T>> getResponse) {
		HttpResponse<String> httpResponse = response().getLeft();
		JsonObject json = response().getRight();
	 ...
	 */
	private <T> void syncCallback(CloudflareCallback<CloudflareResponse<T>> callback,
								  Callable<CloudflareResponse<T>> getResponse) {
	 ...
	 */
	private <T> void asyncCallback(CloudflareCallback<CloudflareResponse<T>> callback,
								   Callable<CloudflareResponse<T>> getResponse) {
